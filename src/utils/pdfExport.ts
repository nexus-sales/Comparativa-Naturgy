...
import { calc, SVE_CATALOG } from './calculations';
...
function pdfPage(
  ...
  const nEn = ({ uni: 1, uni6: 1, tri: 3, tri6: 3, hex: 6 } as Record<string, number>)[tar.tipo] || 1;
  for (let i = 0; i < nEn; i++) {
    const isUniPrice = tar.tipo === 'uni' || tar.tipo === 'uni6';
    const kwh = isUniPrice ? safeEn.reduce((a, v) => a + (+v || 0), 0) : (safeEn[i] || 0);
  ...
  if (potP === 6 && r.reactiva && r.reactiva > 0) {
    ...
    // DESGLOSE
  const sveName = SVE_CATALOG.find(s => s.id === c.sveServiceId)?.name ?? null;
  const sveLine: [string, number, boolean][] = r.sva > 0 && sveName ? [[sveName, r.sva, false]] : [];
  const lineItems: [string | null, number | null, boolean][] = isPyme ? [
    ...sveLine, ['ALQUILER', r.alquiler, false], [null, null, false],
