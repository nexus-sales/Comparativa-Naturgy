  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      ws.eachRow((row, rowNum) => {
        ...
        const accessMatch = c3.match(/^([\d.]+)TD/);
        if (!accessMatch) return; // not a tariff data row

        // C4 contains the exact commercial tariff name, e.g. "Plan Fijo Luz 24h 2.0"
        const baseName = getText(row.getCell(4)).trim();
        ...
        for (let v = 0; v < 3; v++) {
          ...
          const name     = `${baseName}${variantSuffixes[v]}`;
